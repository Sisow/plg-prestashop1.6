...
class Sisow
{
	...
	public function TransactionRequest($keyvalue = NULL) {
		...
		if($productTotal != ($this->amount * 100)){
			...
			$keyvalue['product_description_' . $productId] = $this->translateText('Correctieregel');
	...
	public function AdjustPurchaseId($trxid, $old, $new) {
		...
		$pars["old"] = $old;
		$pars["new"] = $new;
		$pars["sha1"] = sha1($trxid . $old . $new . $this->merchantId . $this->merchantKey);
	...
	protected function translateText($text) {
		...
		switch ($this->payment) {
			case 'sofort':
				$paymentclass = 'sisowde';
				break;
			case 'ebill':
				$paymentclass = 'sisoweb';
				break;
			case 'mistercash':
				$paymentclass = 'sisowmc';
				break;
			case 'overboeking':
				$paymentclass = 'sisowob';
				break;
			case 'paypalec':
				$paymentclass = 'sisowpp';
				break;
			case 'webshop':
				$paymentclass = 'sisowwg';
				break;
			default:
				$paymentclass = 'sisow'.$this->payment;
		}

		$module = Module::getInstanceByName($paymentclass);
