...
class SisowSpraypay extends PaymentModule
{
	public function __construct()
	{
		$this->paymentcode = 'spraypay';
		$this->paymentname = 'Spraypay Gespreid Betalen';
		$this->name = 'sisowspraypay';
	...
	*/
	...
	public function displayForm()
	{
		...
		$fields_form[0]['form'] = array (
			...
			'input' => array (
				...
				array (
					...
					'name' => 'SISOW'.strtoupper($this->paymentcode).'_MINAMOUNT',
					'size' => 28,
					'required' => false
				),
				array (
					...
					'name' => 'SISOW'.strtoupper($this->paymentcode).'_MAXAMOUNT',
					'size' => 28,
					'required' => false
				),
				array(
		...
		if (Tools::isSubmit('submit'.$this->name))
		{
			...
			$orderprefix = (string)Tools::getValue('SISOW'.strtoupper($this->paymentcode).'_PREFIX');
			$orderSuccessState = (string)Tools::getValue('SISOW'.strtoupper($this->paymentcode).'_ORDERSUCCESSTATE');
			$minAmount = (string)Tools::getValue('SISOW'.strtoupper($this->paymentcode).'_MINAMOUNT');
			$maxAmount = (string)Tools::getValue('SISOW'.strtoupper($this->paymentcode).'_MAXAMOUNT');
		}
		else
		{
		...
		$helper->fields_value['SISOW'.strtoupper($this->paymentcode).'_MINAMOUNT'] = $minAmount;
		$helper->fields_value['SISOW'.strtoupper($this->paymentcode).'_MAXAMOUNT'] = $maxAmount;
